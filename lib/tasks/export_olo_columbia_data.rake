...
task :export_olo_columbia_data => [:environment] do
  ...
  CSV.open(data_file, "wb") do |row|
    ...
    OccupationalStatistic.where(area_id: [75,584],
      ...
      row << [stat.year, stat.area.name, stat.industry.code, stat.industry.name, 'average annual wage', (stat.value*52).to_s, 'CES', stat.original_series]
    end
    ...
    OccupationalStatistic.where(area_id: [75,584],
      ...
      row << [stat.year, stat.area.name, stat.industry.code, stat.industry.name, 'employment', stat.value, 'CES', stat.original_series]
    end
  end
end
